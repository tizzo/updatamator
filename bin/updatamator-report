...
[ -e /etc/updatamator/updatamator.conf ] && . /etc/updatamator/updatamator.conf
...
if [ -n "$PKGNAMES" ] ; then
  ...
  for PKG in $PKGNAMES ; do
    ...
    if [ -e ${PKGPATH}${PKG}_${VERFILE}_*.deb ] ; then
...
JSON_STRING="{ \"hostname\": \"$SYSTEM\", \"issue\": $ISSUE, \"secret\": \"$SECRET\", \"updates\": { $JSON_STRING } }"
curl -H "Content-Type: application/json"  -d "$JSON_STRING" -X POST --insecure "${REPORT_HOST}/${REPORT_PATH}"
